...
*}
{capture name=path}<a href="{$link->getPageLink('my-account', true)}">{l s='My account'}</a><span class="navigation-pipe"></span>{l s='My vouchers'}{/capture}
{include file="$tpl_dir./breadcrumb.tpl"}

<h1 class="dotted_bottom">{l s='My vouchers'}</h1>

{if isset($cart_rules) && count($cart_rules) && $nb_cart_rules}
<table class="discount autumn-table responsive table_block">
	<thead>
		<tr>
	...
	{foreach from=$cart_rules item=discountDetail name=myLoop}
		<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
			<td class="discount_code"><span class="table_mobile_label">{l s='Code'}:</span>{$discountDetail.code}</td>
			<td class="discount_description"><span class="table_mobile_label">{l s='Description'}:</span>{$discountDetail.name}</td>
			<td class="discount_quantity"><span class="table_mobile_label">{l s='Quantity'}:</span>{$discountDetail.quantity_for_user}</td>
			<td class="discount_value">
                                <span class="table_mobile_label">{l s='Value'}:</span>
			...
			<td class="discount_minimum">
                                <span class="table_mobile_label">{l s='Minimum'}:</span>
			...
			<td class="discount_cumulative">
                                <span class="table_mobile_label">{l s='Cumulative'}:</span>
			...
			<td class="discount_expiration_date"><span class="table_mobile_label">{l s='Expiration Date'}:</span>{dateFormat date=$discountDetail.date_to}</td>
		</tr>
	{/foreach}
	</tbody>
</table>
<p class="required_info">
