...
{if $products}
	...
	<div class="wishlistLinkTop">
		<a href="#" id="hideSendWishlist" class="autumn-button"  onclick="WishlistVisibility('wishlistLinkTop', 'SendWishlist'); return false;">{l s='Close Details' mod='blockwishlist'}</a>
		<ul class="clearfix display_list">
			<li>
				<a href="#" id="hideBoughtProducts" class="button_account autumn-button"  onclick="WishlistVisibility('wlp_bought', 'BoughtProducts'); return false;">{l s='Hide products' mod='blockwishlist'}</a>
				<a href="#" id="showBoughtProducts" class="button_account autumn-button"  onclick="WishlistVisibility('wlp_bought', 'BoughtProducts'); return false;">{l s='Show products' mod='blockwishlist'}</a>
			</li>
			{if count($productsBoughts)}
			<li>
				<a href="#" id="hideBoughtProductsInfos" class="button_account autumn-button" onclick="WishlistVisibility('wlp_bought_infos', 'BoughtProductsInfos'); return false;">{l s='Hide bought product\'s info' mod='blockwishlist'}</a>
				<a href="#" id="showBoughtProductsInfos" class="button_account autumn-button"  onclick="WishlistVisibility('wlp_bought_infos', 'BoughtProductsInfos'); return false;">{l s='Show bought product\'s info' mod='blockwishlist'}</a>
			</li>
			{/if}
		</ul>
		<p class="wishlisturl">Permalink : <input class="light" type="text" value="{$base_dir_ssl}modules/blockwishlist/view.php?token={$token_wish|escape:'htmlall':'UTF-8'}" readonly="readonly" /></p>
		<p class="submit">
	...
	<div class="wlp_bought">
		<ul class="wlp_bought_list grid group">
			...
			<li id="wlp_{$product.id_product}_{$product.id_product_attribute}" class="clearfix address {if $smarty.foreach.i.index % 2}alternate_{/if}item">
				...
				<div class="clearfix">
					...
					<div class="product_infos">
						<a href="{$link->getProductlink($product.id_product, $product.link_rewrite, $product.category_rewrite)}" title="{l s='Product detail' mod='blockwishlist'}">
                                                    <h5 class="product_name">{$product.name|truncate:28:'...'|escape:'htmlall':'UTF-8'}</h5>
                                                </a>
						<span class="wishlist_product_detail">
							<div class="wishlist_product_detail_qty">
                                                            {l s='Quantity' mod='blockwishlist'}:
                                                            <br /><input type="text" class="light" id="quantity_{$product.id_product}_{$product.id_product_attribute}" value="{$product.quantity|intval}" size="3"  />
							</div>
                                                        
                                                        <div class="wishlist_product_detail_priority">
	...
	<form method="post" class="wl_send std hidden" onsubmit="return (false);">
		<fieldset>
			<p class="required">
				...
				<input type="text" class="light" name="email1" id="email1" />
			</p>
			...
			<p>
				...
				<input type="text" class="light" name="email{$smarty.section.i.index}" id="email{$smarty.section.i.index}" />
			</p>
			{/section}
			<p class="submit">
