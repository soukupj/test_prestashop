...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        $('#scrollable .scroll-content').each(function(){ $(this).jScrollPane();
			...
			<div>
				...
				{foreach from=$filters item=filter}
					{if isset($filter.values)}
						...
						<div id="layered_subtitle_divider" class="clear"></div>
						<ul id="ul_layered_{$filter.type}_{$filter.id_key}" class="scroll-content">
						{if !isset($filter.slider)}
							{if $filter.filter_type == 0}
								{foreach from=$filter.values key=id_value item=value name=fe}
									...
									<li class="nomargin {if $smarty.foreach.fe.index >= $filter.filter_show_limit}hiddable{/if}">
										...
										{/if}
										<label for="layered_{$filter.type_lite}{if $id_value || $filter.type == 'quantity'}_{$id_value}{/if}"{if !$value.nbr} class="disabled"{else}{if isset($filter.is_color_group) && $filter.is_color_group} name="layered_{$filter.type_lite}_{$id_value}" class="layered_color" rel="{$id_value}_{$filter.id_key}"{/if}{/if}>
											{if !$value.nbr}{$value.name|escape:html:'UTF-8'}{if $layered_show_qties}<span> ({$value.nbr})</span>{/if}
											{else}
							...
							{else}
								...
								{foreach from=$filter.values key=id_value item=value name=fe}
									...
									<li class="nomargin {if $smarty.foreach.fe.index >= $filter.filter_show_limit}hiddable{/if}">
										...
										{/if}
										<label for="layered_{$filter.type_lite}{if $id_value || $filter.type == 'quantity'}_{$id_value}{if $filter.id_key}_{$filter.id_key}{/if}{/if}"{if !$value.nbr} class="disabled"{else}{if isset($filter.is_color_group) && $filter.is_color_group} name="layered_{$filter.type_lite}_{$id_value}" class="layered_color" rel="{$id_value}_{$filter.id_key}"{/if}{/if}>
											{if !$value.nbr}{$value.name|escape:html:'UTF-8'}{if $layered_show_qties}<span> ({$value.nbr})</span>{/if}
											{else}
                                                                                            <a href="{$value.link}" rel="{$value.rel}">{$value.name|escape:html:'UTF-8'}{if $layered_show_qties}<span> ({$value.nbr})</span>{/if}</a>
											{/if}
