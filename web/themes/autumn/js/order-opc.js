		...
		success: function(jsonData)
		{
			...
			else
			{
				...
				$('#cart_summary .address_'+deliveryAddress).each(function() {
					...
					if ($(this).find('.cart_unit .price').length > 0 && $(this).find('.cart_unit .price').attr('id').length > 0)
						$(this).find('.cart_unit .price').attr('id', $(this).find('.cart_unit .price').attr('id').replace(/_\d+$/, '_'+idAddress_delivery));

					if ($(this).find('.cart_total .price').length > 0 && $(this).find('.cart_total .price').attr('id').length > 0)
						$(this).find('.cart_total .price').attr('id', $(this).find('.cart_total .price').attr('id').replace(/_\d+$/, '_'+idAddress_delivery));
	...
	$.ajax({
		...
		success: function(json)
		{
			...
			$('#opc_new_account').fadeOut('fast', function() {
				...
				$('#opc_new_account').css('border','none').css('margin',0).css('padding',0);
				// update block user info
				if (json.block_user_info !== '' && $('#header_user_info').length == 1)
				{       
                                       
                                        var $user_info = $(json.block_user_info).not('#header_shopping_cart');
                                        $('#header_user_info').replaceWith($user_info[0]);
		...
		if (guestCheckoutEnabled && !isLogged)
		{
			...
			$('#opc_createAccount').click(function() {
				$('.is_customer_param').show();
                                ...
                                $('#opc_new_account #login_form').hide();
                                //$('#order-opc #new_account_form fieldset').height(1960);
                                $('#order-opc #new_account_form').width('100%');
                                
				$('#opc_account_form').slideDown('slow');
				$('#is_new_customer').val('1');
				$('#opc_account_choice, #opc_invoice_address').hide();
                                
                                $.scrollTo('#opc_new_account', 400);
			...
			$('#opc_guestCheckout').click(function() {
				$('.is_customer_param').hide();
                                ...
                                $('#opc_new_account #login_form').hide();
                                //$('#order-opc #new_account_form fieldset').height(1540);
                                $('#order-opc #new_account_form').width('100%');
