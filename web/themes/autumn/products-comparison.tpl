...
<h1 class="dotted_bottom">{l s='Product Comparison'}</h1>
...
<div class="products_block">
	<table id="product_comparison">
		...
		{foreach from=$products item=product name=for_products}
			...
			<td width="{$width}%" class="comparison_product_details ajax_block_product comparison_infos">
				...
				<h5><a href="{$product->getLink()}" title="{$product->name|truncate:32:'...'|escape:'htmlall':'UTF-8'}">{$product->name|truncate:32:'...'|escape:'htmlall':'UTF-8'}</a></h5>
				<div class="product_desc"><a href="{$product->getLink()}">{$product->description_short|strip_tags|truncate:95:'...'}</a></div>
				...
				{if (!$product->hasAttributes() OR (isset($add_prod_display) AND ($add_prod_display == 1))) AND $product->minimal_quantity == 1 AND $product->customizable != 2 AND !$PS_CATALOG_MODE}
					{if ($product->quantity > 0 OR $product->allow_oosp)}
						<a class="autumn_add_to_cart ajax_add_to_cart_button" rel="ajax_id_product_{$product->id}" href="{$link->getPageLink('cart', true, NULL, "qty=1&amp;id_product={$product->id}&amp;token={$static_token}&amp;add")}" title="{l s='Add to cart'}">{l s='Add to cart'}</a>
					{else}
						<span class="autumn_add_to_cart disabled">{l s='Add to cart'}</span>
					{/if}
				{else}
					<div style="height:23px;"></div>
				{/if}
                                
                                <a class="cmp_remove" href="{$link->getPageLink('products-comparison', true)}" rel="ajax_id_product_{$product->id}">{l s='Remove From Comparison'}</a>
