...
class AutumnNewsletter extends Module
{
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('Autumn Theme - Newsletter');
		$this->description = $this->l('Generates a .CSV file for mass mailings');

		if ($this->id)
		{
			$this->_file = 'export_'.Configuration::get('PS_NEWSLETTER_RAND_AUTUMN').'.csv';
	...
	{
		return (parent::install() AND Configuration::updateValue('PS_NEWSLETTER_RAND_AUTUMN', rand().rand()));
	}
		...
		if (isset($_POST['submitExport']) AND isset($_POST['action']))
		{
			...
			else
			{
				if (!Module::isInstalled('autumnnewsletterblock'))
					$this->_html .= $this->displayError('The module "autumnnewsletterblock" is required for this feature');
				else
					$result = $this->_getAutumnNewsletterBlock();
			}
			...
			elseif ($fd = @fopen(dirname(__FILE__).'/'.strval(preg_replace('#\.{2,}#', '.', $_POST['action'])).'_'.$this->_file, 'w'))
			{
				...
				<a href="../modules/autumnnewsletter/'.Tools::safeOutput(strval($_POST['action'])).'_'.$this->_file.'"><b>'.$this->l('Download the file').' '.$this->_file.'</b></a>
				<br />
	...
	private function _getAutumnNewsletterBlock()
	{
	...
	private function _displayFormExport()
	{
		...
		<p><ol>
			<li>
				'.$this->l('Persons who have subscribed using the AutumnNewsletterBlock block in the front office.').'<br />
				'.$this->l('This is a list of e-mail addresses of persons who come to your store that do not become customers, but have subscribed to your newsletter. Using the "Export Newsletter Subscribers" below will generate a .CSV file based on the AutumnNewsletterBlock subscribers data.').'<br /><br />'.'
			</li>
			<li>
		...
		<fieldset class="width3"><legend>'.$this->l('Export Newsletter Subscribers from the AutumnNewsletterBlock').'</legend>
		<form action="'.Tools::safeOutput($_SERVER['REQUEST_URI']).'" method="post">
			<input type="hidden" name="action" value="autumnNewsletterBlock">
			'.$this->l('Generate a .CSV file based on AutumnNewsletterBlock subscribers data.').' '.$this->l('Only subscribers without an account on the shop will be exported.').'<br /><br />';
		$this->_html .= '<br />
