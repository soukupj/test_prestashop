...
class AutumnNewsletterBlock extends Module
{
	...
	{
		$this->name = 'autumnnewsletterblock';
		$this->tab = 'front_office_features';
		$this->need_instance = 0;

		parent::__construct();

		$this->displayName = $this->l('Autumn Theme - Newsletter Block with Extended Features');
		$this->description = $this->l('Adds a block for newsletter subscription with some modifications.');
	...
	{
		if (parent::install() == false || $this->registerHook('displayFooter') == false || $this->registerHook('header') == false)
			return false;

		Configuration::updateValue('NW_SALT', Tools::passwdGen(16));

		return Db::getInstance()->execute('
		CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'autumn_newsletter` (
	...
	public function uninstall()
	{
		if (!parent::uninstall())
			return false;
		return Db::getInstance()->execute('DROP TABLE '._DB_PREFIX_.'autumn_newsletter');
	}
	...
	public function getContent()
	{
		...
		if (Tools::isSubmit('submitUpdate'))
		{
			if (isset($_POST['new_page']) && Validate::isBool((int)$_POST['new_page']))
				Configuration::updateValue('NW_CONFIRMATION_NEW_PAGE_AUTUMN', $_POST['new_page']);

			if (isset($_POST['conf_email']) && Validate::isBool((int)$_POST['conf_email']))
				Configuration::updateValue('NW_CONFIRMATION_EMAIL_AUTUMN', pSQL($_POST['conf_email']));

			if (isset($_POST['verif_email']) && Validate::isBool((int)$_POST['verif_email']))
				Configuration::updateValue('NW_VERIFICATION_EMAIL_AUTUMN', (int)$_POST['verif_email']);
			...
			{
				Configuration::updateValue('NW_VOUCHER_CODE_AUTUMN', pSQL($_POST['voucher']));
		...
		<form method="post" action="'.Tools::safeOutput($_SERVER['REQUEST_URI']).'">
			<fieldset>
				...
				<div class="margin-form">
					<input type="radio" name="new_page" value="1" '.(Configuration::get('NW_CONFIRMATION_NEW_PAGE_AUTUMN') ? 'checked="checked" ' : '').'/>'.$this->l('yes').'
					<input type="radio" name="new_page" value="0" '.(!Configuration::get('NW_CONFIRMATION_NEW_PAGE_AUTUMN') ? 'checked="checked" ' : '').'/>'.$this->l('no').'
				</div>
				<div class="clear"></div>
				<label>'.$this->l('Send verification e-mail after subscription?').'</label>
				<div class="margin-form">
					<input type="radio" name="verif_email" value="1" '.(Configuration::get('NW_VERIFICATION_EMAIL_AUTUMN') ? 'checked="checked" ' : '').'/>'.$this->l('yes').'
					<input type="radio" name="verif_email" value="0" '.(!Configuration::get('NW_VERIFICATION_EMAIL_AUTUMN') ? 'checked="checked" ' : '').'/>'.$this->l('no').'
				</div>
				<div class="clear"></div>
				<label>'.$this->l('Send confirmation e-mail after subscription?').'</label>
				<div class="margin-form">
					<input type="radio" name="conf_email" value="1" '.(Configuration::get('NW_CONFIRMATION_EMAIL_AUTUMN') ? 'checked="checked" ' : '').'/>'.$this->l('yes').'
					<input type="radio" name="conf_email" value="0" '.(!Configuration::get('NW_CONFIRMATION_EMAIL_AUTUMN') ? 'checked="checked" ' : '').'/>'.$this->l('no').'
				</div>
				<div class="clear"></div>
				<label>'.$this->l('Welcome voucher code').'</label>
				<div class="margin-form">
					<input type="text" name="voucher" value="'.Configuration::get('NW_VOUCHER_CODE_AUTUMN').'" />
	...
	private function newsletterRegistration()
	{
		...
		else if ($_POST['action'] == '1')
		{
			...
			{
				if (!Db::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'autumn_newsletter WHERE `email` = \''.pSQL($_POST['email']).'\' AND id_shop = '.$this->context->shop->id))
		...
		else if ($_POST['action'] == '0')
		{
			...
			{
				if (Configuration::get('NW_VERIFICATION_EMAIL_AUTUMN'))
				{
				...
				else
				{
					...
					if ($code = Configuration::get('NW_VOUCHER_CODE_AUTUMN'))
						$this->sendVoucher($email, $code);

					if (Configuration::get('NW_CONFIRMATION_EMAIL_AUTUMN'))
	...
	{
		$sql = 'INSERT INTO '._DB_PREFIX_.'autumn_newsletter (id_shop, id_shop_group, email, newsletter_date_add, ip_registration_newsletter, http_referer, active)
				VALUES
	...
	{
		return Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'autumn_newsletter`
						SET `active` = 1
						WHERE `email` = \''.pSQL($email).'\''
				);
	}
	...
	public function confirmEmail($token)
	{
		...
		if ($discount = Configuration::get('NW_VOUCHER_CODE_AUTUMN'))
	...
	{
		$verif_url = Context::getContext()->link->getModuleLink('autumnnewsletterblock', 'verification', array(
			'token' => $token,
		));
        ...
        public function hookDisplayFooter($params)
	{
		...
		return $this->display(__FILE__, 'autumnnewsletterblock_footer.tpl');
	}      
	...
	{
		$this->_prepareHook($params);
		return $this->display(__FILE__, 'autumnnewsletterblock.tpl');
	}
        
	public function hookDisplayHeader($params)
	{
		$this->context->controller->addCSS($this->_path.'autumnnewsletterblock.css', 'all');
	}
}
