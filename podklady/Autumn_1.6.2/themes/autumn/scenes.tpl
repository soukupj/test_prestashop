...
<div id="scenes">
	<div>
		...
		<div class="screen_scene" id="screen_scene_{$scene->id}" style="background:transparent url({$base_dir}img/scenes/{$scene->id}-scene_default.jpg); {if !$scene@first} display:none;{/if}">
			...
			{assign var=imageIds value="`$product.id_product`-`$product.id_image`"}
				...
				<div id="scene_products_cluetip_{$scene_key}_{$product_key}_{$product.id_product}" style="display:none;">
					...
					<div class="cluetip_info">
                                            <span class="product_name">{$product.details->name}</span>
