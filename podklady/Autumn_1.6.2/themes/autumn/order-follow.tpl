...
{capture name=path}<a href="{$link->getPageLink('my-account', true)}">{l s='My account'}</a><span class="navigation-pipe"></span>{l s='Return Merchandise Authorization (RMA)'}{/capture}
{include file="$tpl_dir./breadcrumb.tpl"}

<h1 class="dotted_bottom">{l s='Return Merchandise Authorization (RMA)'}</h1>
...
<h4>{l s='Here is a list of pending merchandise returns.'}</h4>
	...
	<table id="order-list" class="std">
		...
		{foreach from=$ordersReturn item=return name=myLoop}
			<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
				<td><span class="table_mobile_label">{l s='Return'}:</span><a class="color-myaccount exclusive" href="javascript:showOrder(0, {$return.id_order_return|intval}, '{$link->getPageLink('order-return', true)}');">{l s='#'}{$return.id_order_return|string_format:"%06d"}</a></td>
				<td class="history_method"><span class="table_mobile_label">{l s='Order'}:</span><a class="color-myaccount exclusive" href="javascript:showOrder(1, {$return.id_order|intval}, '{$link->getPageLink('order-detail', true)}');">{l s='#'}{$return.id_order|string_format:"%06d"}</a></td>
				<td class="history_method"><span class="table_mobile_label">{l s='Package status'}:</span>{$return.state_name|escape:'htmlall':'UTF-8'}</td>
				<td><span class="table_mobile_label">{l s='Date issued'}:</span>{dateFormat date=$return.date_add full=0}</td>
				<td class="history_invoice">
                                <span class="table_mobile_label">{l s='Return slip'}:</span>
				{if $return.state == 2}
