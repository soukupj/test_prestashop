...
// <![CDATA[
	...
	function buildAddressBlock(id_address, address_type, dest_comp)
	{
		...
		li_content['title'] = adr_titles_vals[address_type];
                li_content['update'] = '<a class="exclusive" href="{$link->getPageLink('address', true, NULL, "id_address")}'+id_address+'&amp;back={$back_order_page}&step=1{if $back}&mod={$back}{/if}" title="{l s='Update' js=1}">{l s='Update' js=1}</a>';
...
{/if}
<div id="order_address_wrapper">
{if !$opc}<h1 class="dotted_bottom">{l s='Addresses'}</h1>{else}<h2><span>1</span> {l s='Addresses'}</h2>{/if}
...
{if !$opc}
	...
	{if !$multi_shipping && {Configuration::get('PS_ALLOW_MULTISHIPPING')} && !$cart->isVirtualCart()}
		<div class="button_multishipping_mode" id="multishipping_mode_box">
			<h3 class="title">{l s='Multi-shipping'}</h3>
			<div class="description">
				<a href="{$link->getPageLink('order', true, NULL, 'step=1&multi-shipping=1')}" class="autumn-button">
		...
		<div class="address-form-multishipping">
			<div class="button_multishipping_mode" id="multishipping_mode_box">
				...
				<div class="description_off">
					<a href="{$link->getPageLink('order-opc', true, NULL, 'ajax=1&multi-shipping=1&method=multishipping')}" id="link_multishipping_form" class="autumn-button" title="{l s='Choose the delivery address(es)'}">
						{l s='Choose the delivery addresses'}
					</a>
				</div>
			</div>
...
{/if}
	<div class="addresses clearfix">
		...
		{if $addresses|@count > 1}
			...
			{else}
				<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1&select_address=1{if $back}&mod={$back}{/if}")}" title="{l s='Add'}" class="autumn-button">{l s='Add a new address'}</a>
			{/if}
		</p>
		...
		<p class="address_add submit">
			<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{if $back}&mod={$back}{/if}")}" title="{l s='Add'}" class="autumn-button">{l s='Add a new address'}</a>
		</p>
		{if !$opc}
...
{if !$opc}
	<div class="cart_navigation submit group">
		<input type="hidden" class="hidden" name="step" value="2" />
		<input type="hidden" name="back" value="{$back}" />
		<a href="{$link->getPageLink($back_order_page, true, NULL, "step=0{if $back}&back={$back}{/if}")}" title="{l s='Previous'}" class="exclusive previous">&laquo; {l s='Previous'}</a>
		<input type="submit" name="processAddress" value="{l s='Next'} &raquo;" class="exclusive next" />
	</div>
</form>
{else}
</div>
