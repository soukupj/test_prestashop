...
<h1 class="dotted_bottom">{l s='My loyalty points' mod='loyalty'}</h1>
...
<div class="block-center" id="block-history">
	...
	<table id="order-list" class="autumn-table responsive">
		...
		{foreach from=$displayorders item='order'}
			<tr class="{if $order@iteration is even by 1}alternate_item{/if}">
				<td class="history_link bold"><span class="table_mobile_label">{l s='Order' mod='loyalty'}:</span>{l s='#' mod='loyalty'}{$order.id|string_format:"%06d"}</td>
				<td class="history_date"><span class="table_mobile_label">{l s='Date' mod='loyalty'}:</span>{dateFormat date=$order.date full=1}</td>
				<td class="history_method"><span class="table_mobile_label">{l s='Points' mod='loyalty'}:</span>{$order.points|intval}</td>
				<td class="history_method"><span class="table_mobile_label">{l s='Points Status' mod='loyalty'}:</span>{$order.state|escape:'htmlall':'UTF-8'}</td>
			</tr>
		{/foreach}
		</tbody>
                <tfoot>
			<tr class="alternate_item">
				...
				<td class="history_method">{$totalPoints|intval}</td>
...
<p style="text-align:center; margin-top:20px">
	<a class="exclusive" href="{$link->getModuleLink('loyalty', 'default', ['process' => 'transformpoints'])}" onclick="return confirm('{l s='Are you sure you want to transform your points into vouchers?' mod='loyalty' js=1}');">{l s='Transform my points into a voucher of' mod='loyalty'} <span class="price">{convertPrice price=$voucher}</span>.</a>
</p>
{/if}
...
<h2 class="solid_bottom">{l s='My vouchers from loyalty points' mod='loyalty'}</h2>
...
<div class="block-center" id="block-history">
	<table id="order-list" class="std">
		...
		{foreach from=$discounts item=discount name=myLoop}
			<tr class="alternate_item">
				<td class="history_date"><span class="table_mobile_label">{l s='Created' mod='loyalty'}:</span>{dateFormat date=$discount->date_add}</td>
				<td class="history_price"><span class="table_mobile_label">{l s='Value' mod='loyalty'}:</span><span class="price">{if $discount->reduction_percent > 0}
					...
					{/if}</span></td>
				<td class="history_method bold"><span class="table_mobile_label">{l s='Code' mod='loyalty'}:</span>{$discount->code}</td>
				<td class="history_date"><span class="table_mobile_label">{l s='Valid from' mod='loyalty'}:</span>{dateFormat date=$discount->date_from}</td>
				<td class="history_date"><span class="table_mobile_label">{l s='Valid until' mod='loyalty'}:</span>{dateFormat date=$discount->date_to}</td>
				<td class="history_method bold"><span class="table_mobile_label">{l s='Status' mod='loyalty'}:</span>{if $discount->quantity > 0}{l s='To use' mod='loyalty'}{else}{l s='Used' mod='loyalty'}{/if}</td>
				<td class="history_method">
                                        <span class="table_mobile_label">{l s='Details' mod='loyalty'}:</span>
...
<ul class="footer_links">
	<li><a class="back_account_link" href="{$link->getPageLink('my-account', true)}">{l s='Back to Your Account' mod='loyalty'}</a></li>
	<li class="f_right"><a class="back_home_link" href="{$base_dir}">{l s='Home' mod='loyalty'}</a></li>
</ul>
