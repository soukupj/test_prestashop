...
<div id="mailalerts_block_account">
	<h1 class="dotted_bottom">{l s='My alerts' mod='mailalerts'}</h1>
	{if $mailAlerts}
		<div>
			{foreach from=$mailAlerts item=mailAlert}
			<div class="mailalert clearfix">
				<a href="{$link->getProductLink($mailAlert.id_product, null, null, null, null, $mailAlert.id_shop)}" class="product_img_link"><img src="{$link->getImageLink($mailAlert.link_rewrite, $mailAlert.cover, 'medium_default')}" alt=""/></a>
	...
	<ul class="footer_links">
		<li class="fleft">
                    <a class="back_account_link" href="{$link->getPageLink('my-account', true)}">{l s='Back to Your Account' mod='mailalerts'}</a></li>
	</ul>
</div>