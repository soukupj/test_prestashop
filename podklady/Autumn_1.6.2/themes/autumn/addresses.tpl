...
{capture name=path}<a href="{$link->getPageLink('my-account', true)}">{l s='My account'}</a><span class="navigation-pipe"></span>{l s='My addresses'}{/capture}
{include file="$tpl_dir./breadcrumb.tpl"}

<h1 class="dotted_bottom">{l s='My addresses'}</h1>
<h4>{l s='Please configure your default billing and delivery addresses when placing an order. You may also add additional addresses, which can be useful for sending gifts or receiving an order at your office.'}</h4>

{if isset($multipleAddresses) && $multipleAddresses}
<div class="addresses">
	<h3 class="title">{l s='Your addresses are listed below.'}</h3>
        <p style="margin-top:5px;">{l s='Be sure to update your personal information if it has changed.'}</p>
	{assign var="adrs_style" value=$addresses_style}
	<div class="block_adresses clearfix">
	{foreach from=$multipleAddresses item=address name=myLoop}
		<ul class="address {if $smarty.foreach.myLoop.last}last_item{elseif $smarty.foreach.myLoop.first}first_item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{else}item{/if}">
			...
			{/foreach}
			<li class="address_update"><a class="exclusive" href="{$link->getPageLink('address', true, null, "id_address={$address.object.id|intval}")}" title="{l s='Update'}">{l s='Update'}</a></li>
...
<div class="clear address_add"><a href="{$link->getPageLink('address', true)}" title="{l s='Add an address'}" class="autumn-button">{l s='Add an address'}</a></div>
