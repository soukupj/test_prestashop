...
{/if}
	<div class="addresses clearfix">
		...
		<p class="address_add submit">
			<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{'&multi-shipping=1'|urlencode}{if $back}&mod={$back|urlencode}{/if}")}" title="{l s='Add'}" class="autumn-button">{l s='Add a new address'}</a>
		</p>
		{if !$opc}
...
{if !$opc}
	<div class="cart_navigation submit group">
		...
		{if $back}
			<a href="{$link->getPageLink('order', true, NULL, "step=0&amp;back={$back}")}" title="{l s='Previous'}" class="exclusive previous">&laquo; {l s='Previous'}</a>
		{else}
			<a href="{$link->getPageLink('order', true, NULL, "step=0")}" title="{l s='Previous'}" class="exclusive previous">&laquo; {l s='Previous'}</a>
		{/if}
