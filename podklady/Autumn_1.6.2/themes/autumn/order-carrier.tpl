...
{if !$opc}
	<h1 class="dotted_bottom">{l s='Shipping'}</h1>
{else}
...
{else}
	<h3 class="title">{l s='Choose your delivery method'}</h3>
...
{if $conditions AND $cms_id}
	...
	<p class="checkbox">
		...
		<label for="cgv">{l s='I agree to the '}<a href="{$link_conditions}" class="color iframe">{l s='Terms of Service'}</a>{l s=' and will adhere to them unconditionally.'}</label>
	</p>
...
{if !$opc}
	<p class="cart_navigation submit">
		...
		{if isset($virtual_cart) && $virtual_cart || (isset($delivery_option_list) && !empty($delivery_option_list))}
			<input type="submit" name="processCarrier" value="{l s='Next'} &raquo;" class="exclusive next" />
		{/if}
	</p>
</form>
{else}
