...
<div id="order_detail_header">
    <h3>{l s='Order Reference %s - placed on' sprintf=$order->getUniqReference()} {dateFormat date=$order->date_add full=0}	</h3>

    <form action="{if isset($opc) && $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" class="submit reorder">
                    <input type="hidden" value="{$order->id}" name="id_order"/>
                    <input type="submit" value="{l s='Reorder'}" name="submitReorder" class="exclusive" />
    </form>
</div>
...
<h3 class="solid_bottom">{l s='Follow your order\'s status step-by-step'}</h3>
	...
	<table class="autumn-table responsive">
			...
			{if !isset($product.deleted)}
				...
				{if isset($product.customizedDatas)}
					<tr class="item">
						{if $return_allowed}<td class="order_cb"></td>{/if}
                                                ...
                                                <td>
                                                    <span class="table_mobile_label">{l s='Reference'}:</span>
                                                    <label for="cb_{$product.id_order_detail|intval}">
                                                        {if $product.product_reference}{$product.product_reference|escape:'htmlall':'UTF-8'}{else}--{/if}
                                                    </label>
                                                </td>
                                                ...
                                                <td>
                                                    <span class="table_mobile_label">{l s='Qty'}:</span>
                                                    <input class="order_qte_input"  name="order_qte_input[{$smarty.foreach.products.index}]" type="text light" size="2" value="{$product.customizationQuantityTotal|intval}" />
                                                    <label for="cb_{$product.id_order_detail|intval}">
                                                ...
                                                {if $order->hasProductReturned()}
                                                    <td>
                                                        <span class="table_mobile_label">{l s='Returned'}:</span>
                                                        {$product['qty_returned']}
                                                    </td>
						{/if}
                                                
						<td>
                                                    <span class="table_mobile_label">{l s='Unit price'}:</span>
                                                    <label for="cb_{$product.id_order_detail|intval}">
                                                            {if $group_use_tax}
				...
				{if $product.product_quantity > $product.customizationQuantityTotal}
					<tr class="item">
						...
						<td>
                                                    <span class="table_mobile_label">{l s='Qty'}:</span>
                                                    <input class="order_qte_input" name="order_qte_input[{$product.id_order_detail|intval}]" type="text" size="2" value="{$productQuantity|intval}" />
                                                    <label for="cb_{$product.id_order_detail|intval}">
		...
		{foreach from=$discounts item=discount}
			<tr class="item">
				...
				<td class="description">{l s='Voucher:'} {$discount.name|escape:'htmlall':'UTF-8'}</td>
...
{if $order->getShipping()|count > 0}
	<table class="autumn-table responsive carrier-table">
		<thead>
			<tr>
				<th class="first_item">{l s='Date'}</th>
		...
		<tbody>
			...
			<tr class="item">
				<td><span class="table_mobile_label">{l s='Date'}:</span>{$line.date_add}</td>
                                {* START - Autumn Theme Retro Compatibility Edits *}
				<td><span class="table_mobile_label">{l s='Carrier'}:</span>{if (isset($line.carrier_name))}{$line.carrier_name}{else}{$line.state_name}{/if}</td>
                                {* END - Autumn Theme Retro Compatibility Edits *}
				<td><span class="table_mobile_label">{l s='Weight'}:</span>{if $line.weight > 0}{$line.weight|string_format:"%.3f"} {Configuration::get('PS_WEIGHT_UNIT')}{else}-{/if}</td>
				<td><span class="table_mobile_label">{l s='Shipping cost'}:</span>{if $order->getTaxCalculationMethod() == $smarty.const.PS_TAX_INC}{displayPrice price=$line.shipping_cost_tax_incl currency=$currency->id}{else}{displayPrice price=$line.shipping_cost_tax_excl currency=$currency->id}{/if}</td>
				<td>
					<span class="table_mobile_label">{l s='Tracking number'}:</span><span id="shipping_number_show">{if $line.tracking_number}{if $line.url && $line.tracking_number}<a href="{$line.url|replace:'@':$line.tracking_number}">{$line.tracking_number}</a>{else}{$line.tracking_number}{/if}{else}-{/if}</span>
				</td>
			</tr>
			{/foreach}
		</tbody>
...
{if !$is_guest}
	...
	<div id="returnOrderMessage">
		<h3 class="solid_bottom">{l s='Merchandise return'}</h3>
		<h4>{l s='If you wish to return one or more products, please mark the corresponding boxes and provide an explanation for the return. When complete, click the button below.'}</h4>
		<p class="textarea">
	...
	{if count($messages)}
	<h3 class="solid_bottom">{l s='Messages'}</h3>
	<div class="table_block">
		<table class="detail_step_by_step autumn-table responsive">
                        <colgroup>
                            <col width="1">
                            <col>
                        </colgroup>
			...
			{foreach from=$messages item=message name="messageList"}
				<tr class="{if $smarty.foreach.messageList.first}first_item{elseif $smarty.foreach.messageList.last}last_item{/if} {if $smarty.foreach.messageList.index % 2}alternate_item{else}item{/if}">
					<td>
						<span class="table_mobile_label">{l s='From'}:</span>
					...
					<td><span class="table_mobile_label">{l s='Message'}:</span>{$message.message|escape:'htmlall':'UTF-8'|nl2br}</td>
				</tr>
			{/foreach}
			</tbody>
		</table>
	...
	<form action="{$link->getPageLink('order-detail', true)}" method="post" class="std" id="sendOrderMessage">
		<h3 class="solid_bottom">{l s='Add a message'}</h3>
		<h4>{l s='If you would like to add a comment about your order, please write it in the field below.'}</h4>
		<p>
		...
		<p class="submit">
			...
			<input type="submit" class="exclusive" name="submitMessage" value="{l s='Send'}"/>
		</p>
	</form>
{else}
