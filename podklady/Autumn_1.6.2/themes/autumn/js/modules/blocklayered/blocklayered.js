...
function paginationButton() {
	...
	$('#pagination li').not('.current, .disabled').each(function () {
		var nbPage = 0;
		if ($(this).attr('id') == 'pagination-next')
			nbPage = parseInt($('#pagination li.current').children().html()) + 1;
		else if ($(this).attr('id') == 'pagination-previous')
			nbPage = parseInt($('#pagination li.current').children().html()) - 1;
...
function reloadContent(params_plus)
{
	...
	if (!ajaxLoaderOn)
	{       
                $('#product_list_wrapper').prepend($('<div id="product_list_loader">'));
                $('#product_list_loader').prepend($('#layered_ajax_loader').html());
		//('#product_list').prepend($('#layered_ajax_loader').html());
		$('#product_list_wrapper .grid-container').css('opacity', '0.2');
		ajaxLoaderOn = 1;
	}
		...
		success: function(result)
		{
			...
			if (result.productList){
                            ...
                            if (imageShape == 'rect_img'){
                                result.productList = result.productList.replace(/home_default/g, "rect_default");
                            }
                            
                            $('#product_list_wrapper').replaceWith(utf8_decode(result.productList));
                            
                        }
			else{
                            $('#product_list_wrapper').html('');                            
                        }
			...
			if(slideUp)
                                $.scrollTo('#product_list_wrapper', 400);
			updateProductUrl();
