...
<form action="{$request_uri|escape:'htmlall':'UTF-8'}" method="post" class="std" id="form_forgotpassword">
	<fieldset>
                <h3 class="title">{l s='Forgot your password?'}</h3>
		<p class="text">
			<label for="email">{l s='Email address'}</label>
			<input type="text" class="white" id="email" name="email" value="{if isset($smarty.post.email)}{$smarty.post.email|escape:'htmlall':'UTF-8'|stripslashes}{/if}" />
		</p>
                
		<p class="submit">
			<input type="submit" class="exclusive" value="{l s='Retrieve Password'}" />
		</p>
                
                <p class="back_link">
                    <a href="{$link->getPageLink('authentication')}" title="{l s='Back to login page'}" rel="nofollow">{l s='Back to login page'}</a>
                </p>
	</fieldset>
</form>
{/if}
