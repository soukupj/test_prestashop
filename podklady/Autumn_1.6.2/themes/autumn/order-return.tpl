...
{if isset($orderRet)}
	...
	<div>
                <p class="bold">&bull; {l s='We have logged your return request.'}</p>
		<p>&bull; {l s='Your package must be returned to us within'} {$nbdaysreturn} {l s='days of receiving your order.'}</p>
		<p>&bull; {l s='The current status of your merchandise return is:'} <span class="bold">{$state_name|escape:'htmlall':'UTF-8'}</span></p>
		<p>&bull; {l s='List of items to be returned:'}</p>
	</div>
		...
		<table class="autumn-table responsive">
			...
			{foreach from=$products item=product name=products}
				...
				{foreach from=$returnedCustomizations item='customization' name=products}
					{if $customization.product_id == $product.product_id}
						<tr class="{if $smarty.foreach.products.first}first_item{/if} {if $smarty.foreach.products.index % 2}alternate_item{else}item{/if}">
							...
							<td>{$customization.name|escape:'htmlall':'UTF-8'}</td>
				...
				{if $product.product_quantity > $quantityDisplayed}
					<tr class="{if $smarty.foreach.products.first}first_item{/if} {if $smarty.foreach.products.index % 2}alternate_item{else}item{/if}">
						<td><span class="table_mobile_label">{l s='Reference'}:</span>{if $product.product_reference}{$product.product_reference|escape:'htmlall':'UTF-8'}{else}--{/if}</td>
						<td class="description"><span class="table_mobile_label">{l s='Product'}:</span>{$product.product_name|escape:'htmlall':'UTF-8'}</td>
						<td><span class="table_mobile_label">{l s='Quantity'}:</span><span class="order_qte_span editable">{$product.product_quantity|intval}</span></td>
					</tr>
				{/if}
			{/foreach}
			</tbody>
